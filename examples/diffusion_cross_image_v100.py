...
from MinkowskiEngine.modules.diffusion_block import ResNetBlock,ResNet2,ResNet3
...
from examples.dataset import ModelNet40Dataset, PointCloud, ShapeNetDataset,Objaverse#,collate_pointcloud_fn
from examples.ae_res import save_png, sorted_by_morton_code
from examples.ae_res_v100 import rotate_point_cloud,VaeModule,VAE
from diffusers.models.embeddings import TimestepEmbedding, Timesteps
import MinkowskiEngine.MinkowskiFunctional as MF

from transformers import CLIPProcessor, CLIPModel,CLIPTextModel,CLIPTokenizer,CLIPVisionModel,CLIPImageProcessor
import random
...
def collate_pointcloud_fn(list_data,max_batch_len=1000000):
    ...
    # image_conditions_ids=torch.randint(0,image_conditions.shape[0])
    # Concatenate all lists
    # print("image_conditions: ",image_conditions[0].shape)
    return {
        "coords": ME.utils.batched_coordinates(coords),
        "xyzs": [torch.from_numpy(feat).float() for feat in feats],
        "captions":[f"a picture of a {o_cls}" for o_cls in obj_clss],
        # "image_conditions":[image_condition for image_condition in image_conditions],
        "image_conditions":torch.cat(image_conditions,dim=0),
...
class UNet(nn.Module):
    ...
    def __init__(self,channels,config,inchannel=4,outchannel=4,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None,embedding_dim: Optional[int] = None,with_attn=False):
        ...
        self.block1=self.get_block(ch[0],ch[1],embedding_dim,with_attn,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=self.config.cross_dim)

        self.block2=self.get_block(ch[1],ch[2],embedding_dim,with_attn,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=self.config.cross_dim)

        self.block3=self.get_block(ch[2],ch[3],embedding_dim,with_attn=False,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group)

        self.res_mid=self.get_block(ch[3],ch[3],embedding_dim,with_attn,after=None,num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=self.config.cross_dim)

        self.block3_tr = self.get_block(ch[3], ch[2],embedding_dim,with_attn=False,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group)

        self.block2_tr = self.get_block(ch[2]*2, ch[1],embedding_dim,with_attn,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=self.config.cross_dim)

        self.block1_tr = self.get_block(ch[1]*2, ch[0],embedding_dim,with_attn,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=self.config.cross_dim)
    ...
    def get_block(self,in_channels,out_channels,embedding_dim,with_attn,after=None,num_layers=2,use_conv=True,time_embedding_norm='default',group=1,with_cross_attn=None,cross_attention_dim=None):
        blocks=[]
        for i in range(num_layers):
            if i==0:
                blocks.append(ResNet3(in_channels, out_channels,after=after,embedding_dim=embedding_dim,with_attn=with_attn,use_conv=use_conv,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross_attn,cross_attention_dim=cross_attention_dim)) #RA*n
            else:
                blocks.append(ResNet3(out_channels, out_channels,after=None,embedding_dim=embedding_dim,with_attn=with_attn,use_conv=use_conv,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross_attn,cross_attention_dim=cross_attention_dim))
    ...
    def forward(self, x,timesteps=0,encoder_hidden_state=None):
        ...
        for i in range(len(self.block1)):
            ...
            # print(self.block1[i])
            out_s1 = self.block1[i](out_s1,temb,encoder_hidden_state=encoder_hidden_state)
            # out_s1 = MF.silu(out_s1)
        # print(out_s1.coordinate_map_key)

        out_s2=out_s1
        for i in range(len(self.block2)):
            out_s2 = self.block2[i](out_s2,temb,encoder_hidden_state=encoder_hidden_state)
            # out_s2 = MF.silu(out_s2)
        # print(out_s2.coordinate_map_key)

        out_s3=out_s2
        for i in range(len(self.block3)):
            out_s3 = self.block3[i](out_s3,temb,encoder_hidden_state=encoder_hidden_state)
            # out_s3 = MF.silu(out_s3)
        # print(out_s3.coordinate_map_key)

        out_mid=out_s3
        for i in range(len(self.res_mid)):
            out_mid = self.res_mid[i](out_mid,temb,encoder_hidden_state=encoder_hidden_state)
            # out_mid = MF.silu(out_mid)
        # print(out_mid.coordinate_map_key)

        out = out_mid
        for i in range(len(self.block3_tr)):
            # out = MF.silu(block(out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=out_s2.coordinate_map_key))
            # out=block(out,temb,encoder_hidden_state=encoder_hidden_state)
            if i==len(self.block3_tr)-1:
                out= self.block3_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=out_s2.coordinate_map_key)
            else:
                out= self.block3_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state)
        ...
        for i in range(len(self.block2_tr)):
            # out = MF.silu(block(out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=out_s1.coordinate_map_key))
            # out=block(out,temb,encoder_hidden_state=encoder_hidden_state)
            if i==len(self.block2_tr)-1:
                out=self.block2_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=out_s1.coordinate_map_key)
            else:
                out=self.block2_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state)
        # print(out.coordinate_map_key)

        out = ME.cat(out, out_s1)
        for i in range(len(self.block1_tr)):
            # if i<len(self.block1_tr)-2:
            #     # out = MF.silu(self.block1_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=x.coordinate_map_key))
            #     # out = self.block1_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state)
            #     out=self.block1_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state,coordinates=x.coordinate_map_key)
            # else:
            #     out = self.block1_tr[i](out,temb,encoder_hidden_state=encoder_hidden_state)
...
class diffusionModule(LightningModule):
    def __init__(self,vae,unet,text_encoder,text_tokenizer,image_encoder,config,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None):
        super().__init__()
    ...
    def predict_noise(self, sin,temb,encoder_hidden_state):
        return self.unet(sin,temb,encoder_hidden_state)
    ...
    def training_step(self,batch, batch_idx):
        ...
        # batch["captions"], max_length=self.text_tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        # ).input_ids.to(batch["coords"].device)
        # encoder_hidden_state=self.encoder(input_ids)["last_hidden_state"] #b,77,768
        #image的情况
        encoder_hidden_state=self.image_encoder(batch["image_conditions"])["last_hidden_state"] #b,257,1024
        ...
        model_output = self.predict_noise(noise_clean,timesteps,encoder_hidden_state)
        ...
        loss+=denoise_loss
        # print(loss)
        self.log('denoise_loss',denoise_loss.detach().item(),batch_size=self.config.batch_size, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True)
        # self.log('denoise_loss',denoise_loss.detach().item(),batch_size=self.config.batch_size, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    def on_after_backward(self):
        ...
        for name, param in self.unet.named_parameters():
            gradient = param.grad
            if torch.all(torch.abs(gradient)>1e-6):
                continue
            # 在这里对梯度进行检查或操作
    ...
    def validation_step(self,batch, batch_idx):
        ...
        # print(encoder_hidden_state.shape)
        # print(len(batch["xyzs"]))
        sin = ME.SparseTensor(
            features=torch.ones(len(batch["coords"]), 1).to(batch["coords"].device),
            coordinates=batch["coords"].int(),
        )
        # batch_indices = sin.C[:, 0]
        ...
        for t in self.denoise_scheduler.timesteps:
            ...
            model_output = self.predict_noise(noise_clean,tt,encoder_hidden_state)
            # model_output = self.predict_noise(noise_clean,timesteps,encoder_hidden_state)
...
if __name__ == "__main__":
    ...
    pa.add_argument("--save_every_step",type=int,default=5000,help="save after n step")
    ...
    pa.add_argument("--clip_path", type=str, default="openai/clip-vit-large-patch14", help="clip_path")
    pa.add_argument("--modelnet40_path", type=str, default="/mnt/data4/hjm_code_4/MinkowskiEngine-master/data/ModelNet40", help="modelnet40_path")
    pa.add_argument("--shapenet_path", type=str, default="/mnt/data4/hjm_code_4/MinkowskiEngine-master/data/ShapeNetCore.v2_only_obj", help="shapenet_path")
    pa.add_argument("--cross_dim", type=int, default=1024, help="cross_dim")

    pa.add_argument("--objaverse_path", type=str, default="/root/hjm/data_objaverse/objaverse/hf-objaverse-v1/glbs", help="modelnet40_path")
    pa.add_argument("--objaverse_image_path", type=str, default="/root/hjm/data_objaverse/Objaverse-MIX/rendered_images", help="modelnet40_path")
    pa.add_argument("--vae_channel", type=int,nargs=5, default=(32, 128, 512, 512,4), help="vae channel")
    ...
    train_dataset=ConcatDataset([
            # ModelNet40Dataset(
            #     phase="train",
            #     root=config.modelnet40_path,
            #     # transform=rotate_point_cloud,
            #     with_class=True,
            #     config=config),
            Objaverse(
                # transform=rotate_point_cloud,
                root=config.objaverse_path,
                image_root=config.objaverse_image_path,
                phase="train",
                with_class=True,
                config=config),
            # ShapeNetDataset(
            #     # transform=rotate_point_cloud,
            #     root=config.shapenet_path,
            #     with_class=True,
            #     config=config),
            ])
    ...
    text_encoder = CLIPTextModel.from_pretrained(config.clip_path)
    text_tokenizer = CLIPTokenizer.from_pretrained(config.clip_path)
    vision_encoder=CLIPVisionModel.from_pretrained(config.clip_path)

    diffusion = diffusionModule(vae,unet,text_encoder,text_tokenizer,vision_encoder,config)
    # lr_monitor = LearningRateMonitor(logging_interval='step')
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        # monitor='denoise_loss_epoch', #我们想要监视的指标 
        dirpath=config.floder+'/checkpoints',  #模型缓存目录
        # every_n_train_steps = config.save_every_step, #保存的间隔
        every_n_epochs = config.save_every, #保存的间隔
        save_top_k=-1,
        # filename='diffusion-{epoch:04d}-{denoise_loss_epoch:.5f}'
        filename='diffusion-{epoch:04d}'
    ...
    if config.recover != "scratch":
        if config.recover:
            ckpt_path=config.recover
