...
class UNet(nn.Module):
    ...
    def __init__(self,channels,config,inchannel=4,outchannel=4,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None,embedding_dim: Optional[int] = None,with_attn=False):
        ...
        # self.block1=self.get_block(ch[0],ch[1],embedding_dim,with_attn,after='downsample',num_layers=2)

        # self.block2=self.get_block(ch[1],ch[2],embedding_dim,with_attn,after='downsample',num_layers=2)

        # self.block3=self.get_block(ch[2],ch[3],embedding_dim,with_attn=False,after='downsample',num_layers=2)

        # self.res_mid=self.get_block(ch[3],ch[3],embedding_dim,with_attn,after=None,num_layers=2)

        # self.block3_tr = self.get_block(ch[3], ch[2],embedding_dim,with_attn=False,after='upsample_determine',num_layers=3)

        # self.block2_tr = self.get_block(ch[2]*2, ch[1],embedding_dim,with_attn,after='upsample_determine',num_layers=3)

        # self.block1_tr = self.get_block(ch[1]*2, ch[0],embedding_dim,with_attn,after='upsample_determine',num_layers=3)

        self.block1=self.get_block(ch[0],ch[1],embedding_dim,with_attn,after=None,num_layers=2)

        self.block2=self.get_block(ch[1],ch[2],embedding_dim,with_attn,after=None,num_layers=2)

        self.block3=self.get_block(ch[2],ch[3],embedding_dim,with_attn=False,after=None,num_layers=2)

        self.res_mid=self.get_block(ch[3],ch[3],embedding_dim,with_attn,after=None,num_layers=2)

        self.block3_tr = self.get_block(ch[3], ch[2],embedding_dim,with_attn=False,after=None,num_layers=3)

        self.block2_tr = self.get_block(ch[2]*2, ch[1],embedding_dim,with_attn,after=None,num_layers=3)

        self.block1_tr = self.get_block(ch[1]*2, ch[0],embedding_dim,with_attn,after=None,num_layers=3)

        self.conv_in = ME.MinkowskiConvolution(inchannel,ch[0],3,dimension=3)
    ...
    def get_block(self,in_channels,out_channels,embedding_dim,with_attn,after=None,num_layers=2):
        blocks=[]
        for i in range(num_layers):
            if i==0:
                blocks.append(ResNet3(in_channels, out_channels,after=after,embedding_dim=embedding_dim,with_attn=with_attn))
            else:
                blocks.append(ResNet3(out_channels, out_channels,after=None,embedding_dim=embedding_dim,with_attn=with_attn))
    ...
    def forward(self, x,timesteps=0):
        ...
        for block in self.block3_tr:
            ...
            out= block(out,temb,coordinates=out_s2.coordinate_map_key)
        ...
        for block in self.block2_tr:
            ...
            out=block(out,temb,coordinates=out_s1.coordinate_map_key)
        # print(out.coordinate_map_key)
...
class diffusionModule(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        linearLR = optim.lr_scheduler.LinearLR(optimizer, start_factor=1e-2, end_factor=1.0, total_iters=500)
        cosineAnnealingLR = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5000, eta_min=0)
        scheduler=SequentialLR(optimizer,schedulers=[linearLR,cosineAnnealingLR],milestones=[500])
...
if __name__ == "__main__":
    ...
    unet=UNet(config.unet_channel,config,with_attn=True)
    if config.ngpus > 1:
