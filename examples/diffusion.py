...
class UNet(nn.Module):
    ...
    def forward(self, x,timesteps=0):
        ...
        # for i in range(len(self.block0)):
        #     x = self.block0[i](x,temb)
        # print(x.shape)
        out_s1=x
...
class diffusionModule(LightningModule):
    def __init__(self,vae,unet,config,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None):
        ...
        self.vae_scale=torch.tensor((config.vae_scale))
...
if __name__ == "__main__":
    ...
    pa.add_argument("--vae_scale", type=float, default=0.1428, help="warmup")

    pa.add_argument("--small_dataset", default=False, action='store_true', help="with_attn")
    ...
    unet=UNet(config.unet_channel,config,inchannel=config.vae_channel[-1],outchannel=config.vae_channel[-1],with_attn=config.with_attn)
    if config.ngpus > 1:
