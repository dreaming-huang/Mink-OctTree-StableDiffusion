...
from MinkowskiEngine.modules.diffusion_block_dense import ResNetBlock,ResNet2,ResNet3
...
# from examples.ae_res import ModelNet40Dataset, PointCloud, ShapeNetDataset, 
from examples.ae_res import save_png, sorted_by_morton_code,rotate_point_cloud
from examples.ae_res_v100 import VAE
from diffusers.models.embeddings import TimestepEmbedding, Timesteps
import MinkowskiEngine.MinkowskiFunctional as MF
from examples.dataset import collate_pointcloud_fn,ModelNet40Dataset,PointCloud,ShapeNetDataset
...
class UNet(nn.Module):
    ...
    def __init__(self,channels,config,inchannel=4,outchannel=4,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None,embedding_dim: Optional[int] = None,with_attn=False):
        ...
        # self.block0=self.get_block(ch[0],ch[0],embedding_dim,with_attn,after=None,num_layers=2,time_embedding_norm=time_embedding_norm,group=group)
        ...
        # self.conv_out=nn.Conv3d(ch[0],outchannel,3,padding=1,bias=False,padding_mode="replicate")
    ...
    def forward(self, x,timesteps=0):
        ...
        # x=torch.cat([x,self.sp_3(x),self.sp_5(x)])
        ...
        # print(out.coordinate_map_key)
        out = torch.cat((out, out_s2),dim=1)
        ...
        out = torch.cat((out, out_s1),dim=1)
        ...
        stensor = to_sparse(out)
        new_coords = stensor.C
        new_coords[..., 1:] = new_coords[..., 1:] * sinput.tensor_stride[0]
        ...
        out=self.conv_out(stensor)

        out=sorted_by_morton_code(out)
        # out=sorted_by_morton_code(out)
...
class diffusionModule(LightningModule):
    ...
    def training_step(self,batch, batch_idx):
        ...
        if self.config.prediction_type == "epsilon":
            assert torch.all(model_output.C==noise_clean.C)
    ...
    def validation_step(self,batch, batch_idx):
        ...
        for t in self.denoise_scheduler.timesteps:
            ...
            # print(model_output.C)
            # print(noise_clean.C)
            # print(noise_clean.C.shape)
            # print(model_output.C.shape)
            # noise_clean_C_numpy=clean.C.cpu().numpy()
            # model_output_C_numpy=model_output.C.cpu().numpy()
            # np.savetxt('noise_clean_C_numpy.txt',noise_clean_C_numpy)
            # np.savetxt('model_output_C_numpy.txt',model_output_C_numpy)
            # print((model_output.C!=clean.C).sum())
            assert torch.all(model_output.C==clean.C)
...
if __name__ == "__main__":
    ...
    pa.add_argument("--cache", default=False, action='store_true',help="whether cache pointcloud.npy")
    ...
    #     "openai/clip-vit-large-patch14", subfolder="text_encoder", #revision=args.revision, variant=args.variant
    # )
    # encoder, _ = clip.load("ViT-B/32")
