...
from examples.dataset import ModelNet40Dataset,PointCloud,ShapeNetDataset
...
class UNet(nn.Module):
    ...
    def __init__(self,channels,config,inchannel=1,outchannel=1,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None,embedding_dim: Optional[int] = None,with_attn=False):
...
class diffusionModule(LightningModule):
    ...
    def validation_step(self,batch, batch_idx):
        ...
        sout=noise_clean
        keep_sout = (sout.F > 0.5).squeeze()
        keep_clean= (clean.F>0.5).squeeze()
        pure=ME.MinkowskiPruning()
        if keep_sout.sum() > 1:
            sout=pure(sout,keep_sout)
        clean=pure(clean,keep_clean)
        ...
        if batch_idx==0 and str(self.device) == 'cuda:0':
            ...
            for b, (coords, real_coords) in enumerate(zip(batch_coords, real_batch_coords)):
                ...
                save_png(pcd,opcd,ax,point_size=0.1)
                plt.axis('equal')
                plt.tight_layout()
