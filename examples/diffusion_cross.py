...
from examples.dataset import ModelNet40Dataset, PointCloud, ShapeNetDataset#,collate_pointcloud_fn
...
class UNet(nn.Module):
    ...
    def __init__(self,channels,config,inchannel=4,outchannel=4,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None,embedding_dim: Optional[int] = None,with_attn=False):
        ...
        self.block1=self.get_block(ch[0],ch[1],embedding_dim,with_attn,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=768)

        self.block2=self.get_block(ch[1],ch[2],embedding_dim,with_attn,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=768)

        self.block3=self.get_block(ch[2],ch[3],embedding_dim,with_attn=False,after="downsample",num_layers=2,time_embedding_norm=time_embedding_norm,group=group)

        self.res_mid=self.get_block(ch[3],ch[3],embedding_dim,with_attn,after=None,num_layers=2,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=768)

        self.block3_tr = self.get_block(ch[3], ch[2],embedding_dim,with_attn=False,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group)

        self.block2_tr = self.get_block(ch[2]*2, ch[1],embedding_dim,with_attn,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=768)

        self.block1_tr = self.get_block(ch[1]*2, ch[0],embedding_dim,with_attn,after="upsample",num_layers=3,time_embedding_norm=time_embedding_norm,group=group,with_cross_attn=with_cross,cross_attention_dim=768)
    ...
    def get_block(self,in_channels,out_channels,embedding_dim,with_attn,after=None,num_layers=2,use_conv=True,time_embedding_norm='default',group=1,with_cross_attn=None,cross_attention_dim=768):
        blocks=[]
...
class diffusionModule(LightningModule):
    def __init__(self,vae,unet,encoder,text_tokenizer,config,time_embedding_type="positional",flip_sin_to_cos=True,freq_shift: int = 0,num_train_timesteps: Optional[int] = None):
        super().__init__()
...
if __name__ == "__main__":
    ...
    diffusion = diffusionModule(vae,unet,text_encoder,text_tokenizer,config)
